    ...
    public partial class frmSalesRecord : MetroFramework.Forms.MetroForm
    {
        ...
        private void txtGuestName_TextChanged(object sender, EventArgs e)
        {
            try
            {
                cc.con = new SqlConnection(cs.DBConn);
                cc.con.Open();
                cc.cmd = new SqlCommand("SELECT RTRIM(B_Id) as [Bill ID], RTRIM(BillNo) as [Bill No.], Convert(Datetime,BillDate,131) as [Bill Date],RTRIM(OrderedProduct.CustomerID) as [CId],RTRIM(Customer.CustomerID) as [Customer ID],RTRIM(Name) as [Custome Name], RTRIM(GrandTotal) as [Grand Total], RTRIM(Cash) as [Cash], RTRIM(Change) as [Change] from Customer,OrderedProduct where Customer.C_ID=OrderedProduct.CustomerID and name like '" + txtCustomerName.Text + "%' order by name", cc.con);
                cc.da = new SqlDataAdapter(cc.cmd);
                cc.ds = new DataSet();
        ...
        private void btnGetData_Click(object sender, EventArgs e)
        {
            try
            {
                cc.con = new SqlConnection(cs.DBConn);
                cc.con.Open();
                cc.cmd = new SqlCommand("SELECT RTRIM(B_Id) as [Bill ID], RTRIM(BillNo) as [Bill No.], Convert(Datetime,BillDate,131) as [Bill Date],RTRIM(OrderedProduct.CustomerID) as [CId],RTRIM(Customer.CustomerID) as [Customer ID],RTRIM(Name) as [Custome Name], RTRIM(GrandTotal) as [Grand Total], RTRIM(Cash) as [Cash], RTRIM(Change) as [Change] from Customer,OrderedProduct where Customer.C_ID=OrderedProduct.CustomerID and BillDate between @d1 and @d2 order by BillDate", cc.con);
